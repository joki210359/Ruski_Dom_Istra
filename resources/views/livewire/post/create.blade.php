    ...
    <main class="grid grid-cols-12 gap-3 h-full w-full overflow-hidden">
        ...
        <aside class=" lg:col-span-7  m-auto items-center w-full overflow-scroll">
            ...
            <label for="customFileInput" class=" m-auto max-w-fit flex-col flex gap-3 cursor-pointer">
                <input wire:model.live="media" type="file" multiple accept=".jpg,.png,.jpeg,.mp4,.mov"
                       id="customFileInput" class="sr-only">
            ...
            <div
                ...
                x-on:livewire-upload-finish.window="progress = 100"
                x-on:livewire-upload-error.window="progress = 0"
                class="w-full mt-4">
                <!-- <label for="customFileInput" class="cursor-pointer flex flex-col items-center">
        <input wire:model.live="media"
               type="file"
               multiple
        ...
        <span class="bg-blue-500 text-white text-sm rounded-lg p-2 px-4">
            Upload files
                ...
                <!-- <div class="w-[20rem] ml-[5rem] bg-gray-200 rounded-full h-4 mt-2">
                    <div class="bg-blue-500 h-4 rounded-full transition-all duration-300"
                        :style="`width: ${progress}%`"></div>
                </div> -->
                <div class="w-[20rem] ml-[5rem] bg-white-200 rounded-full h-4 mt-2 relative">
                    <div
                        class="bg-blue-500 h-4 rounded-full transition-all duration-300 flex items-center justify-center text-sm text-white font-semibold"
                ...
                <!-- <p
                    class="text-sm text-black-700 mt-1 ml-[5rem] mr-[5rem] transition-all duration-300"
                    :class="progress <= 20
                            ? 'ml-[5rem] text-left'
                            : (progress >= 50
                            ? 'mr-[5rem] text-right'
        ...
        <aside class=" lg:col-span-5  h-full border-l p-3 flex gap-4 flex-col overflow-hidden overflow-y-scroll">
            ...
            <div class="flex items-center gap-2">
                <x-avatar class="w-9 h-9" src="{{ asset('storage/' . Auth::user()->avatar) }}"/>
                <h5 class="font-bold">
                    <h5 class="font-bold">{{ ucwords(Auth::user()->name) }} {{ ucwords(Auth::user()->familyname) }}</h5>
                </h5>
            </div>
            ...
            <div>
                ...
                <input
                    ...
                    class="border-0 focus:border-0 px-0 w-full rounded-lg bg-white focus:outline-none focus:ring-0 font-semibold text-lg">
            ...
            <div>
                <textarea
                    ...
                    class="border-0 focus:border-0 px-0 w-full rounded-lg bg-white h-24 focus:outline-none focus:ring-0 text-sm"
                    rows="4"></textarea>
