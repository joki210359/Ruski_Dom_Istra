<div wire:key="comment-{{$comment->id}}" class="flex items-center gap-3 py-2">
                 <x-avatar
               src="{{ $comment->user && $comment->user->avatar ? asset('storage/' . $comment->user->avatar) : 'https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_640.png' }}"
...
{{--                      {{ $post->user ? $post->user->familyname : '' }}  --}}
            </span>
                {{$comment->body}}
            </p>
        </div>
        ...
        <div class="col-span-7 flex gap-2 text-sm items-center text-gray-700">
            ...
            <button type="button" wire:click="setParent({{ $comment->id }})" class="font-semibold">Reply</button>
