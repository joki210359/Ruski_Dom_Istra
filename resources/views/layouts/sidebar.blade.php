...
" class="menu p-3   w-20  h-full grid bg-white border-r text-base-content" :class="{'w-72 ':!shrink}">
    ...
    <ul class="space-y-4 mt-2">
        ...
        <li><a wire:navigate href="{{route('reels')}}" class="flex items-center gap-5">
                ...
                <h4 x-cloak x-show="!(shrink||drawer)"
                    class=" text-lg  {{request()->routeIs('reels') ? 'font-bold' : 'font-medium'}}">
                    {{ __('sidebar.reels') }}</h4>
            </a></li>
        ...
        <li><a wire:navigate href="{{route('chat')}}" class="flex items-center gap-5">
                ...
                <h4 x-cloak x-show="!(shrink||drawer)" class=" text-lg font-medium">{{ __('sidebar.messages') }}</h4>
            </a></li>
        ...
        <li>
            <div @click="showSearch=false;showNotifications=true;drawer=true" class="flex items-center gap-5">
                ...
                <h4 x-cloak x-show="!(shrink||drawer)" class=" text-lg font-medium">{{ __('sidebar.notifications') }}
                </h4>
            </div>
        </li>
        ...
        <li>
            <div onclick="Livewire.dispatch('openModal', { component: 'post.create' })" class="flex items-center gap-5">
                ...
                <h4 x-cloak x-show="!(shrink||drawer)" class=" text-lg font-medium">{{ __('sidebar.create') }}</h4>
            </div>
        </li>
        ...
        @auth
            <li>
                <a wire:navigate href="{{ route('profile.home', auth()->user()->username) }}"
                    ...
                    <x-avatar src="{{ url('storage/avatars/' . ltrim(auth()->user()->avatar, 'avatars/')) }}"
    ...
    <footer class="sticky bottom-0 mt-auto w-full grid px-3 z-50 bg-white">
        <div class="dropdown dropdown-top ">
            <label tabindex="0" class=" cursor-pointer bg-white  flex items-center w-full gap-5 m-1">
                ...
                <h3 x-cloak x-show="!(shrink||drawer)" class="text-lg font-medium">{{ __('sidebar.more') }}</h3>
            </label>
            <ul tabindex="0" class="dropdown-content z-[1] menu p-2 space-y-3 shadow bg-base-100 rounded-box w-60">
                ...
                <li><a class="py-2">{{ __('sidebar.settings') }}</a></li>
                <li><a class="py-2">
    ...
    <div x-show="drawer" x-cloak x-transition.origin.left
        ...
        class="fixed inset-y-0 left-[70px] w-72 px-4 overflow-y-scroll overflow-x-hidden shadow bg-white border rounded-r-2xl rounded-l-2xl z-[50]">
        ...
        <template x-if="showSearch">
            ...
            <div class="h-full">
                ...
                <header class="sticky top-0 w-full z-10 bg-white py-2">
                    ...
                    <h5 class="text-4xl font-bold my-4">{{ __('sidebar.search') }}</h5>

                    {{-- input --}}

                    <input wire:model.live="query" type="search" placeholder="{{ __('sidebar.search') }}" class="border-0 outline-none w-64
         bg-gray-100 
         rounded-l-2xl rounded-r-2xl 
         focus:outline-none hover:ring-0 focus:ring-0">
